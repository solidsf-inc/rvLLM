...
// mistralrs-paged-attn/src/metal/kernels/reshape_and_cache.metal
// MIT License, Copyright (c) 2024 Eric Buehler.
// Modification: explicit cache-capacity guard and local include path.
...
[[kernel]] void reshape_and_cache(
    ...
    device const uint &num_cache_blocks [[buffer(13)]],
  ...
  if (block_idx < 0 || uint64_t(block_idx) >= uint64_t(num_cache_blocks)) {
    return;
  }
