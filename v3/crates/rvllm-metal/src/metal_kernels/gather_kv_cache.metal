...
// mistralrs-paged-attn/src/metal/kernels/gather_kv_cache.metal
// MIT License, Copyright (c) 2024 Eric Buehler.
// Modification: device metadata bounds checks and local include path.
...
[[kernel]] void gather_kv_cache(
    ...
    device const uint &num_cache_blocks [[buffer(15)]],
  ...
  if (batch_id < 0 || batch_id >= num_seqs || cu_seq_lens[batch_id] < 0 ||
      cu_seq_lens[batch_id] > token_id ||
      cu_seq_lens[batch_id + 1] <= token_id) {
    for (int i = tid; i < n; i += threads_per_threadgroup) {
      k_out[out_base + i] = OUT_T(0);
      v_out[out_base + i] = OUT_T(0);
    }
    return;
  }
  ...
  if (block_table_id < 0 || block_table_id >= block_table_stride) {
    for (int i = tid; i < n; i += threads_per_threadgroup) {
      k_out[out_base + i] = OUT_T(0);
      v_out[out_base + i] = OUT_T(0);
    }
  ...
  if (block_id < 0 || uint(block_id) >= num_cache_blocks) {
    for (int i = tid; i < n; i += threads_per_threadgroup) {
      k_out[out_base + i] = OUT_T(0);
      v_out[out_base + i] = OUT_T(0);
    }
