...
// mistralrs-paged-attn/src/metal/kernels/pagedattention.metal
//
// License: MIT (mistralrs)
//   See LICENSES/MIT-mistralrs in this repository.
//
// Upstream attribution (preserved below):
//   Portions adapted from Apple's MLX framework (Apache-2.0, © 2023 Apple Inc.)
//   See LICENSES/Apache-2.0-mlx in this repository.
//   Portions adapted from the vLLM project (Apache-2.0).
//
// Modifications:
//   - Renamed `#include "utils.metal"` to `#include "pagedattn_utils.metal"`
//   - Added cache-block and context bounds before device pointer arithmetic.
// ============================================================================
...
[[kernel]] void paged_attention(
    ...
    const constant uint &num_cache_blocks [[buffer(19)]],
    const constant uint &max_context_len [[buffer(20)]],
  ...
  const uint64_t table_capacity =
      uint64_t(max_num_blocks_per_seq) * uint64_t(BLOCK_SIZE);
  const int head_idx = threadgroup_position_in_grid.x;
  const int num_heads = threadgroups_per_grid.x;

  // Validate device-resident metadata before any cache pointer arithmetic.
  threadgroup atomic_uint invalid_mapping;
  if (thread_idx == 0) {
    atomic_store_explicit(&invalid_mapping, 0u, memory_order_relaxed);
    if (context_len == 0 || max_num_blocks_per_seq <= 0 ||
        uint64_t(context_len) > table_capacity ||
        context_len > max_context_len) {
      atomic_store_explicit(&invalid_mapping, 1u, memory_order_relaxed);
    }
  }
  threadgroup_barrier(mem_flags::mem_threadgroup);
  if (atomic_load_explicit(&invalid_mapping, memory_order_relaxed) == 0u) {
    const int blocks_to_check = DIVIDE_ROUND_UP(context_len, BLOCK_SIZE);
    const device uint32_t *validated_table =
        block_tables + seq_idx * max_num_blocks_per_seq;
    for (int block_idx = thread_idx; block_idx < blocks_to_check;
         block_idx += NUM_THREADS) {
      if (validated_table[block_idx] >= num_cache_blocks) {
        atomic_store_explicit(&invalid_mapping, 1u, memory_order_relaxed);
      }
    }
  }
...
#pragma unroll
      ...
      // Apply softcapping. Non-positive means disabled; the Rust wrapper maps
      // `None` to 0.0 so the no-softcap path cannot divide by zero.
      if (softcapping > 0.0) {
