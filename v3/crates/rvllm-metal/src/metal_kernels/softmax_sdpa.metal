// softmax_sdpa.metal
//
// Combined softmax-with-sinks and SDPA-with-sinks kernels for Metal.
//
// Adapted from mistral.rs revision 31c13eb4587d3e4a5204870c98b70c05a1e5c943:
// mistralrs-quant/src/metal_kernels/softmax_with_sinks.metal
// mistralrs-quant/src/metal_kernels/sdpa_with_sinks.metal
//
// License: MIT (mistralrs upstream, Copyright (c) 2024 Eric Buehler).
//          See LICENSES/MIT-mistralrs for the full license text.
//
// Modifications:
//   - Combined the two source files into a single translation unit.
//   - Shared common include.
//   - Function constant 50 compiles a true no-sink path when no sink tensor
//     is supplied; no virtual probability mass is added in that variant.
...
constant bool rvllm_use_attention_sinks [[function_constant(50)]];
...
[[kernel]] void softmax_with_sinks(
  ...
  if (rvllm_use_attention_sinks) {
    sink_val = float(sinks[h]);
  }
  ...
  if (rvllm_use_attention_sinks && tid == 0) {
